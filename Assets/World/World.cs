...
public class World : Singleton<World>
{
    ...
    public void neighborLightUpdate(Chunk c, int x, int y, int z, LightLevel newLight)
    {
        LightLevel nextLevel = new LightLevel(0, 0, 0);
        LightLevel thisLevel = c.getLightLevel(x, y, z);

        bool queue = false;

        nextLevel.r = Math.Max(newLight.r, thisLevel.r);
        nextLevel.g = Math.Max(newLight.g, thisLevel.g);
        nextLevel.b = Math.Max(newLight.b, thisLevel.b);

        if (thisLevel.r != nextLevel.r)
        {
            nextLevel.r = newLight.r;
            queue = true;
        }
        if (thisLevel.g != nextLevel.g)
        {
            nextLevel.g = newLight.g;
            queue = true;
        }
        if (thisLevel.b != nextLevel.b)
        {
            nextLevel.b = newLight.b;
            queue = true;
        }

        if (c.getBlock(x,y,z) == 0 && queue)
        {
            c.setLightLevel(nextLevel, x, y, z);
    ...
    public void UpdateLight()
    {
        while (lightBfsQueue.Count > 0)
        {
            ...
            LightLevel light = c.getLightLevel(x, y, z);
            byte currentRed = light.r;
            byte currentGreen = light.g;
            byte currentBlue = light.b;

            byte intensityRed = currentRed;
            byte intensityGreen = currentGreen;
            byte intensityBlue = currentBlue;
            ...
            LightLevel nextLight = new LightLevel(intensityRed, intensityGreen, intensityBlue);
            ...
            //debugCubes.Add(new Vector4(x + (CHUNK_WIDTH * c.position.x), y, z + (CHUNK_LENGTH * c.position.y), (int)intensityRed));
           // Bobby.Log("I: " + intensityRed);

            if (x > 0)
            {
                neighborLightUpdate(c, x - 1, y, z, nextLight);
            }
            else if (negX != null)
            {
                neighborLightUpdate(negX, CHUNK_WIDTH - 1, y, z, nextLight);
            }
            ...
            {
                neighborLightUpdate(posX, 0, y, z, nextLight);
            }

            if (z > 0)
            {
                neighborLightUpdate(c, x, y, z - 1, nextLight);
            }
            else if (negZ != null)
            {
                neighborLightUpdate(negZ, x, y, CHUNK_LENGTH - 1, nextLight);
            }
            ...
            {
                neighborLightUpdate(posZ, x, y, 0, nextLight);
            }

            if(y > 0)
            {
                neighborLightUpdate(c, x, y - 1, z, nextLight);
            }
